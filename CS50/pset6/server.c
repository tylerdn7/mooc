...
char* indexes(const char* path)
{
    // for size of index.php declaring an integer variable length of size of the path string length
    // plus 10
    
    int length = strlen(path) + 10; 
    
    // for storing index.php a char pointer php and mallocking it with 
    // the size of length variable
    
    char* php = malloc(length);
    
    // using strcpy to copy the path string into php
    // using strcat to appending the strings
    
    strcpy(php, path);
    strcat(php, "index.php");
    
    // if php is NULL then returning false
    ...
    else if (access(php, F_OK) != -1)
    {
       return php;
    }
    
    // as index.html is sized 11 so incrementing the size by 1
    
    length += 1;
    // for storing index.html a char pointer html and mallocking it with 
    // the size of length variable
    
    char* html = malloc(length);
    
    // using strcpy to copy the path string into html
    // using strcat to appending the strings
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    // read from file 1 byte at a time into string
    while (fread(string, size, elements, file) != 0)
    {
        // appending bytes to content by reallocating space
        *content = realloc(*content, *length + size + 1);
        memcpy(*content + *length, string, size);
        *length += size;

        // null-terminate for ending the string
        *(*content + *length) = '\0';
    }
...
const char* lookup(const char* path)
{
    // integer n holding the size of path string
    
    int n = strlen(path);
    
    if(path[n-1] == 's' && path[n-2] == 's' && path[n-3] == 'c')
    {
         // if path string ends with .css then returning text/css
        return "text/css";
    }
    if(path[n-1] == 'l' && path[n-2] == 'm' && path[n-3] == 't' && path[n-4] == 'h')
    {
        // if path string ends with .html then returning text/html
        return "text/html";
    }
    if(path[n-1] == 'L' && path[n-2] == 'M' && path[n-3] == 'T' && path[n-4] == 'H')
    {
    ...
    if(path[n-1] == 'f' && path[n-2] == 'i' && path[n-3] == 'g')
    {
        // if path string ends with .gif then returning text/gif
        return "image/gif";
    }
    if(path[n-1] == 'o' && path[n-2] == 'c' && path[n-3] == 'i')
    {
        // if path string ends with .ico then returning image/x-icon
        return "image/x-icon";
    }
    if(path[n-1] == 'g' && path[n-2] == 'p' && path[n-3] == 'j')
    {
        // if path string ends with .jpg then returning image/jpeg
        return "image/jpeg";
    }
    if(path[n-1] == 's' && path[n-2] == 'j')
    {
        // if path string ends with .js then returning text/javascript
        return "text/javascript";
    }
    if(path[n-1] == 'p' && path[n-2] == 'h' && path[n-3] == 'p')
    {
        // if path string ends with .php then returning text/x-php
        return "text/x-php";
    }
    if(path[n-1] == 'g' && path[n-2] == 'n' && path[n-3] == 'p')
    {
        // if path string ends with .png then returning image/png
        return "image/png";
    }
    return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // variable length contains the size of line string
    
    int length = strlen(line);
    
    // incrementing the size by 1 for null terminator
    char str[length + 1]; 
    
    // copying the string line into str string
    
    strcpy(str, line);
    
    // string getstr contains the string upto 1st space
    
    const char* getstr = strtok(str, " "); 
    ...
    // string str2 contains the string upto 2nd space
    
    const char* str2 = strtok(NULL, " ");
    
    // checking whether the string str2 is null or not
    ...
    const char* str3 = strtok(NULL,"\r\n");
    
    char version[15] = "HTTP/1.1";
    
    // check for http version
    
    if(strcasecmp(version,str3) == 1)
    {
        error(505);
        return false;
    }
    
    int strings_length = strlen(str2);
    
    char final_string[strings_length + 1]; 
    
    // copying the str2 string intro final_string
    
    strcpy(final_string, str2);
    
    // if str2 doesn't have a null then copying it ot abs_path string
    
    if (strchr(str2, '?') == NULL)
    {
        
        strcpy(abs_path, final_string);
        query = "";
    }
    // but if request str2 has contain a ? 
    else
    {
        // then creating a const char pointer which contains the string upto ?
        // and query2 containg after the ?'s rest string
        
        const char* abs_path2 = strtok(final_string, "?");
        const char* query2 = strtok(NULL, "?");
        if (query2 == NULL)
        {
            query2 = "";
        }
        
        // copying abs_path2 string into abs_path
        // copying query2 string into query
        
        strcpy(abs_path, abs_path2);
        strcpy(query, query2);
    }
    return true;
